...
{
  char *path = (char *)user_va_to_pa((pagetable_t)(current->pagetable), (void *)vpath);
  return do_rcwd(path);
}
...
void relative_path_to_absolute_path(char *relative_path, char *absolute_path)
{
  ...
  if (strlen(relative_path) == 0) // relative_path is empty
  {
    return;
  }
  else if (relative_path[0] == '/') // not a relative_path
  {
    return;
  }
  if (strcmp("/", current->pfiles->cwd->name) != 0)
  {
    strcpy(absolute_path, current->pfiles->cwd->name);
  }
  while (*ap != '\0')
  {
    ap++;
  ...
  while (*rp != '\0')
  {
    ...
    *t = '\0';
    ...
    else if (strcmp(".", tmp) != 0) // parse other
    {
      *ap = '/';
      ap++;
      t = tmp;
  ...
  *ap = '\0';
  ...
  // sprint("relative_path is %s\n", relative_path);
  // sprint("absolute_path is %s\n", absolute_path);
