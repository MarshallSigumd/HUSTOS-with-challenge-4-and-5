  ...
  for( int i=0; i<parent->total_mapped_region; i++ ){
    ...
    switch( parent->mapped_info[i].seg_type ){
      ...
      case CODE_SEGMENT:
        ...
        uint64 va=parent->mapped_info[i].va,pa=lookup_pa(parent->pagetable, parent->mapped_info[i].va),size=parent->mapped_info[i].npages*PGSIZE;
        map_pages(child->pagetable, va, size, pa, prot_to_type(PROT_READ | PROT_EXEC, 1));
