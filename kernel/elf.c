...
elf_status elf_load(elf_ctx *ctx)
{
    ...
    elf_sect_header section_header1, section_header2;
    elf_prog_header ph_addr;
    int i, off;

    uint64 addr_tail = 0; // tail of address
    char segment_name[SEG_NAME_MAX_LEN];
    ((elf_info *)ctx->info)->p->debugline = NULL;
    // load name section
    if (elf_fpread(ctx, (void *)&section_header1, sizeof(section_header1),
                   ctx->ehdr.shoff + ctx->ehdr.shstrndx * sizeof(section_header1)) != sizeof(section_header1))
    {
        return EL_EIO;
    }
    ...
    for (i = 0, off = ctx->ehdr.phoff; i < ctx->ehdr.phnum; i++, off += sizeof(ph_addr))
    {
        ...
        // update addr_tail
        if (addr_tail < ph_addr.vaddr + ph_addr.memsz)
        {
            addr_tail = ph_addr.vaddr + ph_addr.memsz;
        }
    }
    for (i = 0, off = ctx->ehdr.shoff; i < ctx->ehdr.shnum; i++, off += sizeof(section_header2))
    {
        if (elf_fpread(ctx, (void *)&section_header2, sizeof(section_header2), off) != sizeof(section_header2))
        {
            return EL_EIO;
        }
        elf_fpread(ctx, (void *)segment_name, 20, section_header1.offset + section_header2.name);
        if (strcmp(".debug_line", segment_name) == 0)
        {
            if (elf_fpread(ctx, (void *)addr_tail, section_header2.size, section_header2.offset) != section_header2.size)
            {
                return EL_EIO;
            }
            make_addr_line(ctx, (char *)addr_tail, section_header2.size);
            break;
        }
    }
