...
elf_status elf_load(elf_ctx *ctx)
{
  ...
  for (i = 0, off = ctx->ehdr.shoff; i < ctx->ehdr.shnum; i++, off += sizeof(sh_addr))
  {
    ...
    {
      if (elf_fpread(ctx, &ctx->symbols, sh_addr.size, sh_addr.offset) != sh_addr.size)
        return EL_EIO;
      ctx->symbol_num = sh_addr.size / sizeof(elf_symbol);
    }
    else if (sh_addr.type == STR_TABLE)
    {
      if (elf_fpread(ctx, &ctx->string_table + string_length, sh_addr.size, sh_addr.offset) != sh_addr.size)
        return EL_EIO;
      string_length += sh_addr.size; // there may be several string tables
    }
  }
